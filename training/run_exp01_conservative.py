"""
PRAHARI-AI — Phase 5: Experiment 01 (Conservative Fine-Tune)
...
- Output: runs/detect/prahari_a4_exp01/
"""
...
def evaluate_candidate_on_val(model_path, conf=0.35, imgsz=640):
    ...
    print(f"[*] Evaluating candidate model on validation split: {model_path} (conf={conf}, imgsz={imgsz})")
...
def main():
    print("=" * 80)
    print("PRAHARI-AI — PHASE 5: EXPERIMENT 01 (CONSERVATIVE FINE-TUNE)")
    print("=" * 80)
    ...
    # 2. Train model
    print(f"[*] Starting YOLO fine-tuning: epochs=50, imgsz=640, batch=16, seed=42, patience=20...")
    t0 = time.time()
    try:
        ...
        with open("reports/checkpoints/A4_EXP01_FAILED.md", "w") as f:
            f.write(f"# PRAHARI-AI — Checkpoint: A4_EXP01_FAILED\n\nError: {e}\nTraceback:\n{traceback.format_exc()}\n")
        sys.exit(1)
    ...
    # Parse results.csv for Ultralytics validation stats & epochs
    ...
    # 5. Run rigorous validation split evaluation with PRAHARI pipeline matching (conf=0.35, imgsz=640)
    val_eval = evaluate_candidate_on_val(best_pt, conf=0.35, imgsz=640)
    print("\n" + "=" * 70)
    print("EXPERIMENT 01 VALIDATION EVALUATION (conf=0.35, imgsz=640):")
    ov = val_eval["overall"]
    ...
    # 6. Load baseline validation numbers for comparison
    ...
    # 7. Write completion checkpoint
    os.makedirs("reports/checkpoints", exist_ok=True)
    complete_md = f"""# PRAHARI-AI — Checkpoint: A4_EXP01_COMPLETE
...
Precision: {ultralytics_p:.4f}
Recall: {ultralytics_r:.4f}
F1: {2 * ultralytics_p * ultralytics_r / (ultralytics_p + ultralytics_r) if (ultralytics_p + ultralytics_r) > 0 else 0.0:.4f}
mAP50: {ultralytics_map50:.4f}
mAP50-95: {ultralytics_map50_95:.4f}

Validation evaluation metrics (Standardized conf=0.35, imgsz=640 bipartite matching):
Precision: {ov['precision']:.4f}
Recall: {ov['recall']:.4f}
F1: {ov['f1']:.4f}
...
"""
    with open("reports/checkpoints/A4_EXP01_COMPLETE.md", "w", encoding="utf-8") as f:
        f.write(complete_md)
    print(f"[+] Checkpoint written to reports/checkpoints/A4_EXP01_COMPLETE.md")

    # 8. Write validation comparison report
    ...
    decision = "PROMISING" if diff_f1 > 0.05 and val_eval["per_class"]["person"]["f1"] >= base_pc["person"]["f1"] else ("NOT PROMISING" if diff_f1 >= 0 else "REJECTED")

    val_comparison_md = f"""# PRAHARI-AI — Phase A4: Experiment 01 Validation Comparison

**Date**: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}  
**Candidate**: `runs/detect/prahari_a4_exp01/weights/best.pt` (SHA256: `{best_sha}`)  
...
| :--- | :---: | :---: | :---: | :---: |
| **Precision** | {base_ov['precision']:.4f} | {ov['precision']:.4f} | {'+' if diff_p >= 0 else ''}{diff_p:.4f} | {'+' if diff_p >= 0 else ''}{(diff_p / base_ov['precision'] * 100):.1f}% |
| **Recall** | {base_ov['recall']:.4f} | {ov['recall']:.4f} | {'+' if diff_r >= 0 else ''}{diff_r:.4f} | {'+' if diff_r >= 0 else ''}{(diff_r / base_ov['recall'] * 100):.1f}% |
| **F1-Score** | **{base_ov['f1']:.4f}** | **{ov['f1']:.4f}** | **{'+' if diff_f1 >= 0 else ''}{diff_f1:.4f}** | **{'+' if diff_f1 >= 0 else ''}{(diff_f1 / base_ov['f1'] * 100):.1f}%** |
| **mAP50** (Ultralytics) | N/A (COCO weights) | {ultralytics_map50:.4f} | Baseline COCO: N/A | Direct Domain Adaptation |
| **mAP50-95** (Ultralytics) | N/A (COCO weights) | {ultralytics_map50_95:.4f} | Baseline COCO: N/A | Direct Domain Adaptation |
| **Person F1** | {base_pc['person']['f1']:.4f} | {val_eval['per_class']['person']['f1']:.4f} | {'+' if person_diff >= 0 else ''}{person_diff:.4f} | {'+' if person_diff >= 0 else ''}{(person_diff / base_pc['person']['f1'] * 100):.1f}% |
| **Car F1** | {base_pc['car']['f1']:.4f} | {val_eval['per_class']['car']['f1']:.4f} | {'+' if car_diff >= 0 else ''}{car_diff:.4f} | {'+' if car_diff >= 0 else ''}{(car_diff / base_pc['car']['f1'] * 100):.1f}% |
| **Motorcycle F1** | {base_pc['motorcycle']['f1']:.4f} | {val_eval['per_class']['motorcycle']['f1']:.4f} | {'+' if moto_diff >= 0 else ''}{moto_diff:.4f} | {'+' if moto_diff >= 0 else ''}{(moto_diff / base_pc['motorcycle']['f1'] * 100):.1f}% |
| **Truck F1** | {base_pc['truck']['f1']:.4f} | {val_eval['per_class']['truck']['f1']:.4f} | {'+' if truck_diff >= 0 else ''}{truck_diff:.4f} | {'+' if truck_diff >= 0 else ''}{(truck_diff / base_pc['truck']['f1'] * 100):.1f}% |
| **Hard-Negative FP/image** | {base_hn['fp_per_image']:.4f} | {hn['fp_per_image']:.4f} | {'+' if (hn['fp_per_image'] - base_hn['fp_per_image']) >= 0 else ''}{(hn['fp_per_image'] - base_hn['fp_per_image']):.4f} | {hn['false_positives']} FP across 28 images |

---

## 2. Counts Summary

- **Production Baseline Counts**: TP={base_ov['tp']}, FP={base_ov['fp']}, FN={base_ov['fn']}
- **Experiment 01 Counts**: TP={ov['tp']}, FP={ov['fp']}, FN={ov['fn']}
- **False Positive Reduction**: {base_ov['fp']} -> {ov['fp']} ({'-' if ov['fp'] < base_ov['fp'] else '+'}{abs(ov['fp'] - base_ov['fp'])} FPs)
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **CAM-01** (Gate/Road) | {base_val['per_camera']['CAM-01']['f1']:.4f} | {val_eval['per_camera']['CAM-01']['f1']:.4f} | {val_eval['per_camera']['CAM-01']['f1'] - base_val['per_camera']['CAM-01']['f1']:+.4f} | {val_eval['per_camera']['CAM-01']['tp']} | {val_eval['per_camera']['CAM-01']['fp']} | {val_eval['per_camera']['CAM-01']['fn']} |
| **CAM-02** (Compound) | {base_val['per_camera']['CAM-02']['f1']:.4f} | {val_eval['per_camera']['CAM-02']['f1']:.4f} | {val_eval['per_camera']['CAM-02']['f1'] - base_val['per_camera']['CAM-02']['f1']:+.4f} | {val_eval['per_camera']['CAM-02']['tp']} | {val_eval['per_camera']['CAM-02']['fp']} | {val_eval['per_camera']['CAM-02']['fn']} |
| **CAM-03** (Night IR) | {base_val['per_camera']['CAM-03']['f1']:.4f} | {val_eval['per_camera']['CAM-03']['f1']:.4f} | {val_eval['per_camera']['CAM-03']['f1'] - base_val['per_camera']['CAM-03']['f1']:+.4f} | {val_eval['per_camera']['CAM-03']['tp']} | {val_eval['per_camera']['CAM-03']['fp']} | {val_eval['per_camera']['CAM-03']['fn']} |
| **CAM-04** (Perimeter) | {base_val['per_camera']['CAM-04']['f1']:.4f} | {val_eval['per_camera']['CAM-04']['f1']:.4f} | {val_eval['per_camera']['CAM-04']['f1'] - base_val['per_camera']['CAM-04']['f1']:+.4f} | {val_eval['per_camera']['CAM-04']['tp']} | {val_eval['per_camera']['CAM-04']['fp']} | {val_eval['per_camera']['CAM-04']['fn']} |

---

## 4. Analytical Conclusion & Decision

**Result**: **EXP01 = {decision}**

Experiment 01 fine-tuning successfully domain-adapted the production YOLOv8n detector onto PRAHARI-AI Dataset V2.
The model weights remain isolated in `{best_pt}`. Production weights `weights/yolov8n.pt` are completely untouched.
"""

    with open("reports/PHASE_A4_EXP01_VALIDATION.md", "w", encoding="utf-8") as f:
        f.write(val_comparison_md)
    print(f"[+] Validation comparison written to reports/PHASE_A4_EXP01_VALIDATION.md")
    ...
    with open("reports/exp01_validation_result.json", "w", encoding="utf-8") as f:
        json.dump(res_json, f, indent=2)

    print("\n[+] EXPERIMENT 01 PIPELINE COMPLETED SUCCESSFULLY!")
