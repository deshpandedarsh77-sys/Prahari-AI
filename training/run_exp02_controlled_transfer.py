"""
PRAHARI-AI — Phase 5: Experiment 02 (Controlled Transfer Learning)
...
- Batch: 16
- Epochs: 50
- Learning rate: lr0=0.001 (Reduced by 10x vs EXP01)
- Freeze: 10 (Freezes backbone layers 0..9: Stem, Conv, C2f, SPPF; neck & head trainable)
- Seed: 42
- Patience: 20
- Device: 0 (RTX 3050 Laptop GPU)
- Output: runs/detect/prahari_a4_exp02/
"""
...
def evaluate_candidate_on_val(model_path, conf=0.35, imgsz=640):
    ...
    print(f"[*] Evaluating EXP02 candidate model on validation split: {model_path} (conf={conf}, imgsz={imgsz})")
...
def main():
    print("=" * 80)
    print("PRAHARI-AI — PHASE 5: EXPERIMENT 02 (CONTROLLED TRANSFER LEARNING)")
    print("=" * 80)
    ...
    # 2. Train model with freeze=10 and lr0=0.001
    print(f"[*] Starting Controlled Transfer Training: epochs=50, imgsz=640, batch=16, lr0=0.001, freeze=10, seed=42, patience=20...")
    t0 = time.time()
    try:
        ...
        with open("reports/checkpoints/A4_EXP02_FAILED.md", "w") as f:
            f.write(f"# PRAHARI-AI — Checkpoint: A4_EXP02_FAILED\n\nError: {e}\nTraceback:\n{traceback.format_exc()}\n")
        sys.exit(1)
    ...
    # 5. Run validation evaluation matching standardized PRAHARI protocol (conf=0.35, imgsz=640)
    val_eval = evaluate_candidate_on_val(best_pt, conf=0.35, imgsz=640)
    print("\n" + "=" * 70)
    print("EXPERIMENT 02 VALIDATION EVALUATION (conf=0.35, imgsz=640):")
    ov = val_eval["overall"]
    ...
    # 6. Load baseline & EXP01 validation numbers for comparison
    ...
    if diff_f1 > 0.05 and val_eval["per_class"]["person"]["f1"] >= base_pc["person"]["f1"]:
        decision = "PROMISING"
    elif ov["f1"] < 0.15:
        decision = "REJECTED"
    elif diff_f1 > 0:
    ...
    # 8. Write completion checkpoint
    os.makedirs("reports/checkpoints", exist_ok=True)
    complete_md = f"""# PRAHARI-AI — Checkpoint: A4_EXP02_COMPLETE
...
Freeze configuration: freeze=10 (Backbone layers 0..9 frozen)

Validation:

Production Precision: {base_ov['precision']:.4f}
EXP02 Precision: {ov['precision']:.4f}

Production Recall: {base_ov['recall']:.4f}
EXP02 Recall: {ov['recall']:.4f}
...
{"Proceed to validation comparison and candidate scorecard analysis." if decision == "PROMISING" else "Analyze root cause of transfer learning bottleneck and evaluate resolution / capacity adjustments."}
"""
    with open("reports/checkpoints/A4_EXP02_COMPLETE.md", "w", encoding="utf-8") as f:
        f.write(complete_md)
    print(f"[+] Checkpoint written to reports/checkpoints/A4_EXP02_COMPLETE.md")
    ...
    val_comparison_md = f"""# PRAHARI-AI — Phase A4: Experiment 02 Validation Comparison
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Precision** | {base_ov['precision']:.4f} | {exp01_ov['precision']:.4f} | {ov['precision']:.4f} | {diff_p:+.4f} | {'Improved' if diff_p > 0 else 'Degraded'} |
| **Recall** | {base_ov['recall']:.4f} | {exp01_ov['recall']:.4f} | {ov['recall']:.4f} | {diff_r:+.4f} | {'Improved' if diff_r > 0 else 'Degraded'} |
| **F1-Score** | **{base_ov['f1']:.4f}** | **{exp01_ov['f1']:.4f}** | **{ov['f1']:.4f}** | **{diff_f1:+.4f}** | **{'Improved' if diff_f1 > 0 else 'Degraded'}** |
| **mAP50** (Ultralytics) | N/A | {exp01_val['ultralytics_metrics']['map50']:.4f} | {ultralytics_map50:.4f} | N/A | Domain Evaluation |
| **mAP50-95** (Ultralytics) | N/A | {exp01_val['ultralytics_metrics']['map50_95']:.4f} | {ultralytics_map50_95:.4f} | N/A | Domain Evaluation |
| **Person F1** | {base_pc['person']['f1']:.4f} | {exp01_pc['person']['f1']:.4f} | {val_eval['per_class']['person']['f1']:.4f} | {person_diff:+.4f} | {'Improved' if person_diff > 0 else 'Degraded'} |
| **Car F1** | {base_pc['car']['f1']:.4f} | {exp01_pc['car']['f1']:.4f} | {val_eval['per_class']['car']['f1']:.4f} | {car_diff:+.4f} | {'Improved' if car_diff > 0 else 'Degraded'} |
| **Motorcycle F1** | {base_pc['motorcycle']['f1']:.4f} | {exp01_pc['motorcycle']['f1']:.4f} | {val_eval['per_class']['motorcycle']['f1']:.4f} | {moto_diff:+.4f} | {'Improved' if moto_diff > 0 else 'Degraded'} |
| **Truck F1** | {base_pc['truck']['f1']:.4f} | {exp01_pc['truck']['f1']:.4f} | {val_eval['per_class']['truck']['f1']:.4f} | {truck_diff:+.4f} | {'Improved' if truck_diff > 0 else 'Degraded'} |
| **Hard-Negative FP/image** | {base_hn['fp_per_image']:.4f} | {exp01_val['validation_eval']['hard_negatives']['fp_per_image']:.4f} | {hn['fp_per_image']:.4f} | {hn['fp_per_image'] - base_hn['fp_per_image']:+.4f} | {hn['false_positives']} FP across 28 images |
...
- **Experiment 02**: TP={ov['tp']}, FP={ov['fp']}, FN={ov['fn']}
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **CAM-01** (Gate/Road) | {base_val['per_camera']['CAM-01']['f1']:.4f} | {exp01_val['validation_eval']['per_camera']['CAM-01']['f1']:.4f} | {val_eval['per_camera']['CAM-01']['f1']:.4f} | {val_eval['per_camera']['CAM-01']['f1'] - base_val['per_camera']['CAM-01']['f1']:+.4f} | {val_eval['per_camera']['CAM-01']['tp']} | {val_eval['per_camera']['CAM-01']['fp']} | {val_eval['per_camera']['CAM-01']['fn']} |
| **CAM-02** (Compound) | {base_val['per_camera']['CAM-02']['f1']:.4f} | {exp01_val['validation_eval']['per_camera']['CAM-02']['f1']:.4f} | {val_eval['per_camera']['CAM-02']['f1']:.4f} | {val_eval['per_camera']['CAM-02']['f1'] - base_val['per_camera']['CAM-02']['f1']:+.4f} | {val_eval['per_camera']['CAM-02']['tp']} | {val_eval['per_camera']['CAM-02']['fp']} | {val_eval['per_camera']['CAM-02']['fn']} |
| **CAM-03** (Night IR) | {base_val['per_camera']['CAM-03']['f1']:.4f} | {exp01_val['validation_eval']['per_camera']['CAM-03']['f1']:.4f} | {val_eval['per_camera']['CAM-03']['f1']:.4f} | {val_eval['per_camera']['CAM-03']['f1'] - base_val['per_camera']['CAM-03']['f1']:+.4f} | {val_eval['per_camera']['CAM-03']['tp']} | {val_eval['per_camera']['CAM-03']['fp']} | {val_eval['per_camera']['CAM-03']['fn']} |
| **CAM-04** (Perimeter) | {base_val['per_camera']['CAM-04']['f1']:.4f} | {exp01_val['validation_eval']['per_camera']['CAM-04']['f1']:.4f} | {val_eval['per_camera']['CAM-04']['f1']:.4f} | {val_eval['per_camera']['CAM-04']['f1'] - base_val['per_camera']['CAM-04']['f1']:+.4f} | {val_eval['per_camera']['CAM-04']['tp']} | {val_eval['per_camera']['CAM-04']['fp']} | {val_eval['per_camera']['CAM-04']['fn']} |

---

## 4. Analytical Conclusion & Decision

**Result**: **EXP02 = {decision}**

Experiment 02 tested controlled transfer learning with backbone freezing (`freeze=10`) and reduced learning rate (`lr0=0.001`).
The candidate model weights remain strictly isolated in `{best_pt}`.
Production weights `weights/yolov8n.pt` are completely untouched.
"""

    with open("reports/PHASE_A4_EXP02_VALIDATION.md", "w", encoding="utf-8") as f:
        f.write(val_comparison_md)
    print(f"[+] Validation comparison written to reports/PHASE_A4_EXP02_VALIDATION.md")
    ...
    with open("reports/exp02_validation_result.json", "w", encoding="utf-8") as f:
        json.dump(res_json, f, indent=2)

    print("\n[+] EXPERIMENT 02 PIPELINE COMPLETED SUCCESSFULLY!")
